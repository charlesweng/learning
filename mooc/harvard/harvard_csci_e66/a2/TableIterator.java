...
public class TableIterator extends RelationIterator {
    ...
    public boolean next() throws DeadlockException, DatabaseException {
        ...
        OperationStatus ret = (this.numTuples == 0) ?
        		this.cursor.getFirst(this.key, this.data, null) :
        		this.cursor.getNext(this.key, this.data, null);
        if (ret == OperationStatus.NOTFOUND)
            return false;
    ...
    public Object getColumnVal(int colIndex) {
    	TupleInput data = new TupleInput(this.data.getData());
    	
    	data.mark(0);
    	int[] offsets = new int[this.table.numColumns() + 1];
    	...
    	if (col.isPrimaryKey()) {
        	TupleInput key = new TupleInput(this.key.getData());
    		return readObject(col, key, key.getBufferLength());
    		
    	} else if (offsets[colIndex] < 0) {
    		return null;
    	} else {
    		int begin = offsets[colIndex];
    		int end = offsets[colIndex + 1];
    		for (int i = colIndex + 1; end < 0 && colIndex < this.table.numColumns(); i++) {
    			end = offsets[i];
    		}
    		
    		data.skip(begin);
    		return readObject(col, data, end-begin);
    	}
    }
    
    private Object readObject(Column c, TupleInput data, int length) {
    	Object result = null;
    	switch(c.getType()) {
    	case Column.INTEGER:
    		result = data.readInt();
    	...
    	case Column.CHAR:
    	case Column.VARCHAR:
    		result = (String) data.readBytes(length);
    	}
    	return result;
    }
