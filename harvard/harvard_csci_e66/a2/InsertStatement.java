...
public class InsertStatement extends SQLStatement {
    ...
    public void execute() throws DatabaseException, DeadlockException {
        try {
            ...
            OperationStatus statusCode = null;
            
            if (row.getKey() == null) {
            	DatabaseEntry recnoKey = new DatabaseEntry();
            	table.getDB().append(null, recnoKey, row.getData());
            	TupleInput rI = new TupleInput(recnoKey.getData());
            } else {
            	if (table.getDB().exists(null, row.getKey()) == OperationStatus.SUCCESS) {
                	throw new Exception("Primary key: " + row.getKeyStr() + " already exists.");
            	}
            	table.getDB().putNoOverwrite(null, row.getKey(), row.getData());
            }
